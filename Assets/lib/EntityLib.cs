...
public static class PlayerUtils{
	public static void ShootPlayer<T>(this GameObject obj, float range, float damage){
		if(typeof(T) == typeof(RaycastHit)){
			if(obj.ShootPlayer(range, out RaycastHit hit)){
	...
	// This doesnt actually shoot the player, it just checks to see if the player is in range and if so hits them
	public static bool ShootPlayer(this GameObject obj, float range, out RaycastHit hit){
		Vector3 playerPos = mas.player.Player.transform.position;
		if(Physics.Raycast(obj.transform.position, playerPos-obj.transform.position, out hit, range)){
			return hit.isEntity<Player.PL_Controller>();
		}
